...
data "aws_iam_policy_document" "demo-codepipeline-role-policy" {
  statement {
    ...
    resources = [
      aws_s3_bucket.code_pipeline_artifacts.arn,
      "${aws_s3_bucket.code_pipeline_artifacts.arn}/*",
    ]
  }
  statement {
  ...
  statement {
    ...
    resources = [
      "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/node-codepipeline",
    ]
  }
  statement {
    effect = "Allow"
  ...
  statement { ## REVISAR LA CONDICION
    ...
    resources = [
      ...
      "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/null",
    ]
    condition {
      test     = "StringLike"
